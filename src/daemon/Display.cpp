...
namespace SDDM {
	...
	//vein finger login
       ...
       // if (user == QLatin1String("sddm")) {
         //   return;
       // }
        //fingervein authenticate
/*
            if (verify_Data("verify",3)){
        // authenticate
        qDebug() << "Veinfinger :"<<verify_Data("verify",3);

//        startAuth(user, password, session);
                                        }
        else{
        qDebug() << "Veinfinger failed"<<verify_Data("verify",3);
        return;
            }       
//	qDebug() << "usrname"<<user;
...
//	 qDebug() << "usrname"<<user;
  //      qDebug() << "passwd"<<password;
    //    qDebug() << "session"<<session;
        ...
        // authenticate
	if(1==verify_Data("verify",3)){
        startAuth(user, password, session);
	 qDebug() << "Veinfinger and Passwd verify Success :"<<verify_Data("verify",3);
					}
	else{
	 qDebug() << "Veinfinger Login Fail :"<<verify_Data("verify",3);
	return;}
    }
 ...
 /*定义函数*/
	...
	qDebug() << "Veinfinger starting.......";
        ...
        QString user = QStringLiteral("jack");
        QString password = QStringLiteral("jack");
